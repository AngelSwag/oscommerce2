...
if (SIMPLESUGGEST_ACTIVE != 'false' && (isset($_GET['keywords']) && $_GET['keywords'] != '')) {
  ...
  if (SMARTSUGGEST_RESULT == 'Product Names') {
    if (SMARTSUGGEST_PRODUCTS_SORT == 'Product Names') {
      $suggest_query = tep_db_query("SELECT pd.products_name as result, p.products_image
                                     FROM " . TABLE_PRODUCTS . " p LEFT JOIN " . TABLE_PRODUCTS_DESCRIPTION . " pd USING(products_id)
                                     WHERE $where_str
                                     ORDER BY pd.products_name
                                     LIMIT " . SMARTSUGGEST_LIMIT);
    } else {
      $suggest_query = tep_db_query("SELECT pd.products_name as result, p.products_image, k.keywords, count(*) as count_searched, count(orders_id IS NOT NULL) as count_ordered
  ...
  while ($suggest = tep_db_fetch_array($suggest_query)) {
    ...
    if (SMARTSUGGEST_RESULT == 'Product Names' && SMARTSUGGEST_SHOW_PRODUCT_IMAGE != 'false' )
        echo '<img src="' . DIR_WS_IMAGES . $suggest['products_image'] . '" height="64px"  /> ';
    echo $display_name;
    echo "\n";
  }
}
