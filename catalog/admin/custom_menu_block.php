...
<table border="0" width="100%" cellspacing="0" cellpadding="2">
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            <td class="pageHeading">Custom Menu Block content</td>
    ...
    <?php
        ...
        $check = tep_db_query("select * from `custom_menu_block` ");
        ...
        if (isset($_POST['menu_block_content1'])) {
            ...
            while ($languages_output = tep_db_fetch_array($languages )) {
                ...
                //if (isset($_POST['menu_block_content'.$language_number])) {
                    ...
                    $page_query = tep_db_query("select menu_block_content from `custom_menu_block` where `menu_block_id` = '$language_number' ");
                    $page_count = tep_db_num_rows($page_query);
                    if ($page_count > 0) {
                        tep_db_query("UPDATE `custom_menu_block` SET  `menu_block_content` =  '$menu_block_content' where `menu_block_id` = '$language_number' ");
                    } else {
                        tep_db_query("INSERT INTO `custom_menu_block` (`menu_block_content`, `menu_block_id`) VALUES ('$menu_block_content', '$language_number')  ");
                    }
            }
            ...
            Changes are done!
                <a href="custom_menu_block.php" style="float: right;margin-top: 10px" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary ui-priority-secondary" role="button" aria-disabled="false">
    ...
    <form name="menu_block_content_language" action="custom_menu_block.php" method="post">
          ...
          <tr>
            <td valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
              <tr class="dataTableHeadingRow">
                <td class="dataTableHeadingContent">ENTER ANY CONTENT FOR THE MENU CUSTOM BLOCK BELOW</td>
              </tr>
                <tr>
            ...
            $languages_count = tep_db_num_rows($languages1);
                    ...
                    while ($languages_output = tep_db_fetch_array($languages1 )) {
                        $language_number = $languages_output['languages_id'];
                                    $new_page_query = tep_db_query("select menu_block_content from `custom_menu_block` where `menu_block_id` = '$language_number' ");
