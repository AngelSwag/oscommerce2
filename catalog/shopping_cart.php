    ...
    // Push all attributes information in an array
          if (isset($products[$i]['attributes']) && is_array($products[$i]['attributes'])) {
            while (list($option, $value) = each($products[$i]['attributes'])) {
              ...
              //$attributes = tep_db_query("select popt.products_options_name, poval.products_options_values_name, pa.options_values_price, pa.price_prefix QT PRO
	          ...
	          $products[$i][$option]['track_stock'] = $attributes_values['products_options_track_stock']; // QT PRO  
            }
          }
        }
    ?>
    ...
    <?php
        ...
        for ($i=0, $n=sizeof($products); $i<$n; $i++) {
          ...
          $products_name = '<table border="0" cellspacing="2" cellpadding="2" class="cart_item">' .
                           '  <tr>' .
                           '    <td align="left"><a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $products[$i]['id']) . '">' . tep_image(DIR_WS_IMAGES . $products[$i]['image'], $products[$i]['name'], '', '', 'class="shoppingcart_img"') . '</a></td>' .
                           '    <td valign="top" align="left"><div  class="shopping_cart_product_name"><a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $products[$i]['id']) . '"><strong>' . $products[$i]['name'] . '</strong></a></div>';
...
//++++ QT Pro: End Changed Code
          ...
          echo '        <td valign="top">' . $products_name . '</td>' .
               ...
               <strong>' . $currencies->display_price($products[$i]['final_price'], tep_get_tax_rate($products[$i]['tax_class_id']), $products[$i]['quantity']) . '</strong>
                         <div class="shopping_cart_button_remove">
