    ...
    function execute() {
      ...
      if ($random_product = tep_random_select("select products_id, products_image, products_tax_class_id, products_price from " . TABLE_PRODUCTS . " where products_status = '1' order by products_date_added desc limit " . MAX_RANDOM_SELECT_NEW)) {
        ...
        if (tep_not_null($random_product['specials_new_products_price'])) {
          ...
          $whats_new_price .= '<span class="new-special-price">' . $currencies->display_price($random_product['specials_new_products_price'], tep_get_tax_rate($random_product['products_tax_class_id'])) . '</span>';
        } else {
          $whats_new_price = $currencies->display_price($random_product['products_price'], tep_get_tax_rate($random_product['products_tax_class_id']));
        }

          $whats_new_price = '<span class="new_price">'.$whats_new_price.'</span>';

          $data = '<div class="infoBox infoBoxWhatsNew">' .
                '  <div class="infoBoxHeading"><p class="icon_box_new">&nbsp;</p><a href="' . tep_href_link(FILENAME_PRODUCTS_NEW) . '">' . MODULE_BOXES_WHATS_NEW_BOX_TITLE . '</a></div>' .
                '  <div class="infoBoxContents" style="text-align: center;">

                        <div class="product">
                            <div class="img_border">
                                <div class="product-image-wrapper">
                                    <a class="product_img" href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $random_product["products_id"]) . '">
                            ...
                            <div class="wrapper-hover">
                                ...
                                <div class="clearfix">
                                   <div class="icon_cart_title product-price">'.$whats_new_price.'</div>
                                    <div class="icon_cart_rollover">
                                           <a href="' . tep_href_link(basename('shopping_cart.php'), tep_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $random_product['products_id']) . '"></a>
                                    </div>
                                </div>
                            </div>
                        </div>
