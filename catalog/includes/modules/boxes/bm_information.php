  ...
  class bm_information {
    ...
    function execute() {
      ...
      $data = '<div class="infoBox infoBoxInformation">' .
			  ...
			  '    <a href="' . tep_href_link(FILENAME_GV_FAQ) . '">' . MODULE_BOXES_INFORMATION_GV . '</a>' . //CCGV
              '  </div>' .
              '</div>';
    ...
    function install() {
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Content Placement', 'MODULE_BOXES_INFORMATION_CONTENT_PLACEMENT', 'Right Column', 'Should the module be loaded in the left or right column?', '6', '1', 'tep_cfg_select_option(array(\'Left Column\', \'Right Column\'), ', now())");
